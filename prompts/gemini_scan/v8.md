...
rationale = """
v7 produced great rubric distributions (1-10 per dim) and surfaced
strong candidates, but the 3-second cut for compose-selected was
centered on the geometric midpoint of the 10-20s clip window. For a
clip like "emerging from canopy to NYC skyline reveal," the visual
peak isn't at the middle — it's wherever the reveal lands. The
midpoint cut often misses the moment.

v8 adds a sixth field, peak_offset (0.0-1.0), where Gemini picks
the best 3-second window inside the clip:
  - peak_offset = 0.0 → 3-second cut starts at the clip start
  - peak_offset = 0.5 → 3-second cut centered at the clip middle
  - peak_offset = 1.0 → 3-second cut ends at the clip end

The composer uses this to compute anchor_video_secs precisely.
"""
+++
